...
import static org.example.spring1.UrlMapping.ASSETS;
  ...
  public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    http.csrf(CsrfConfigurer::disable)
        ...
        .authorizeHttpRequests(request ->
            request
                    .requestMatchers(HttpMethod.GET, ASSETS).permitAll() // Everyone can read assets

                    .requestMatchers(AUTH + "/**").permitAll() // localhost:8080/api/auth/...

                    // Only admins can modify assets
                    .requestMatchers(HttpMethod.POST, ASSETS).hasRole("ADMIN")
                    .requestMatchers(HttpMethod.PUT, ASSETS).hasRole("ADMIN")
